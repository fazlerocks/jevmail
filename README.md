...
The screen is an inbox progress bar over five tray icons (Apple-style, one per category) and a Superhuman-style list with a reading pane. Press Fetch emails: the bar fills as Gmail hands over mail, then Jev sorts everything in one run while each email's preview rides the bar, passes the Jev checkpoint, and drops into its tray. Keyboard: `j` `k` move, `e` done, `o` open in Gmail, `esc` close.
...
| `pnpm sort:reset` | Forget the sorting and corrections but keep fetched messages, so the next Sync re-sorts without touching Gmail |
